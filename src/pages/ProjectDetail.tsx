...
const ProjectDetail: React.FC = () => {
  ...
  const handleShareProject = async (platform?: string) => {
    ...
    const description =
      ...
      project?.description ||
      "Check out this project on Garflock!";

    if (platform) {
      let url = "";
      switch (platform) {
        case "twitter":
          url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(shareUrl)}&hashtags=consparium,project`;
          break;
        case "facebook":
  ...
  useEffect(() => {
    if (project) {
      ...
      const description = project.description || "An amazing project on Garflock";

      document.title = `${title} - Garflock`;
