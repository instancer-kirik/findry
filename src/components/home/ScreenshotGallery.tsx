...
const ScreenshotGallery: React.FC<ScreenshotGalleryProps> = ({ screenshots }) => {
  ...
  // Group screenshots into desktop and mobile
  const desktopScreenshots = screenshots.filter((_, index) => aspectRatios[index] && aspectRatios[index] >= 1);
  const mobileScreenshots = screenshots.filter((_, index) => aspectRatios[index] && aspectRatios[index] < 1);
